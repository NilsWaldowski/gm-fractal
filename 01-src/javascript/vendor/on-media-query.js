...
(function(omq, window, document, undefined) { // rgne.ws/16pSbHc
    ...
    omq.init = function(query_array) {
        ...
        this._context      = ''; // Current active query to be read inside callbacks, as `this._last_context`
        // won't be set when they're called!
    ...
    omq.addQuery = function(query_object) {
        ...
        if (query_object) {
            ...
            if (typeof query_object.context === 'string') {
                ...
                // If the context is passed as a string then turn it into an array
                // (for unified approach elsewhere in the code):
            ...
            if (size && key) {
                ...
                for (i = 0, l = this._callbacks.length; i < l; i++) {
                    ...
                    if (this._callbacks[i].call_for_each_context === false) {
                        ...
                        if (((key === 'match') && _inArray(this._last_context, this._callbacks[i].context)) ||
                            ((key === 'unmatch') && _inArray(this._context, this._callbacks[i].context))) {
    ...
    //--------------------------------------------------------------------
        ...
        _pickContext = function() {
            ...
            // Android browsers place a "," after an item in the font family list; most browsers either single or
    ...
    //--------------------------------------------------------------------
         ...
         *
         * @param elem { object } The Element that is the root element of the document (for example, the
         * `<html>` element for HTML documents).
         * @return { string } Value or empty string.
         * @see https://github.com/JoshBarr/js-media-queries/pull/16
         */
        ...
        _fontFamily = function(elem) {
            ...
            if (elem) {
                ...
                // return (IE browser?) ? (Return IE fontFamily) : ((W3C browser?) ? (Return W3C font-family)
                // : Return empty string);
                return (elem.currentStyle) ? elem.currentStyle.fontFamily : ((window.getComputedStyle) ?
    ...
    //--------------------------------------------------------------------
        ...
        _contentAfter = function(elem) {
            ...
            if (elem) {
                ...
                return (window.getComputedStyle) ? window.getComputedStyle(elem, ':after')
                    .getPropertyValue('content') : ''; // getPropertyValue() Returns an empty string in
                // Firefox and Opera and null in Google Chrome and Safari.
...
}((window.oMQ = window.oMQ || {}), window, document, undefined)); // rgne.ws/V4HOQy
